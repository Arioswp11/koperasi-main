...
class Transaksi extends BaseController
{
    ...
    public function simpan()
    {
        ...
        if (session()->get('logged_in') == TRUE && session()->get('admin') == TRUE) {
            $data['dashboard'] = $this->db1->query(
                "SELECT SUM(case when MONTH(tgl_transaksi)=MONTH(CURDATE()) AND deleted_at IS NULL then nominal else 0 end) as simpan_bulan, 
                SUM(case when deleted_at IS NULL then nominal else 0 end) as simpan_total FROM tc_simpan"
            )->getResultArray();
    ...
    public function simpandetail($id)
    {
        ...
        // Ambil data anggota
        $data['anggota'] = $this->anggotaModels->where('id', $id)->first();
    ...
    public function simpancreate()
    {
        ...
        $add = array(
            'tgl_transaksi' => date('Y-m-d H:i:s', strtotime($this->request->getPost('tgl_transaksi'))),
            'nominal' => $this->request->getPost('nominal'),
            'id_anggota' => $this->request->getPost('id_anggota')
        );
        if ($validation->run($add, 'simpanadd') == FALSE) {
            session()->setFlashdata('error', 'Data gagal ditambahkan. Silakan periksa kembali input Anda.');
    ...
    public function simpanedit($id)
    {
        ...
        if ($validation->run($add, 'simpanadd') == FALSE) {
            ...
            if ($simpan) {
                ...
                session()->setFlashdata('error', 'Data gagal diperbarui. Silakan coba lagi.');
                return view('pages/simpancreate', $data);
            }
        }
    }
